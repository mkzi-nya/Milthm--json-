...
def load_beatmap_dict(dict_file):
 ...
 with open(dict_file,'r',encoding='utf-8') as file:
  for line in file:
   ...
   if not line:
    continue
   match = re.match(r'"([a-f0-9\-]+)":\s*{\s*constant:\s*([\d.]+),\s*category:\s*"([^"]+)",\s*name:\s*"([^"]+)"\s*}',line)
   if match:
    beatmap_id,constant,category,name = match.groups()
...
def convert_save_format(input_file,output_file,beatmap_dict):
 ...
 converted_data = {
  "UserName": username,
  "UserUUID": "a5206f20-986e-4164-9883-85fd2cee5e0",
